...
const TypingGame: React.FC<Props> = ({ settings, onAdminRequest }) => {
    ...
    if (stage === 1) {
      ...
      if (questionCount > 0 && questionCount % 4 === 3) {
        ...
        if (settings.isRandomMode) {
          // ランダムモード
          let availableChars = availableCharacters.filter(char => 
            char !== 'F' && char !== 'J' && char !== lastCharacter
          );
          if (availableChars.length === 0) {
            availableChars = availableCharacters.filter(char => 
              char !== 'F' && char !== 'J'
            );
          }
          text = availableChars[Math.floor(Math.random() * availableChars.length)];
        } else {
          // 通常モード
          let availableChars = currentSet.filter(char => 
            char !== 'F' && char !== 'J' && char !== lastCharacter
          );
          if (availableChars.length === 0) {
            availableChars = currentSet.filter(char => 
              char !== 'F' && char !== 'J'
            );
          }
          text = availableChars[Math.floor(Math.random() * availableChars.length)];
  ...
  }, [stage, questionCount, speedMultiplier, settings.isRandomMode, availableCharacters, lastCharacter]);
      ...
      <div className="w-full max-w-2xl p-8 bg-gradient-to-b from-blue-100 to-blue-200 shadow-xl rounded-lg transition-transform duration-300 hover:scale-[1.02]">
        <div className="text-center mb-4">
          <div className="flex justify-between items-center mb-2">
            <h1 className="text-2xl font-bold">typingTaro r01</h1>
            <button
          ...
          <div className="flex justify-between items-center px-4">
            ...
            <div>
              <p className={`text-lg transform transition-all duration-300 ${scoreAnimation ? 'scale-125 text-green-600' : ''}`}>
                スコア: {score}
              </p>
        ...
        <div className={`relative h-96 bg-gradient-to-b ${stageBackgrounds[stage as keyof typeof stageBackgrounds]} rounded-lg mb-8 overflow-hidden ${shakeAnimation ? 'animate-[shake_0.5s_ease-in-out]' : ''}`}>
          {gameState === 'start' && (
          ...
          {gameState === 'stageClear' && (
            <div className="absolute inset-0 bg-gradient-to-b from-emerald-300 via-green-400 to-emerald-500 flex flex-col items-center justify-center text-white">
              ...
              <Trophy className="mb-4 text-yellow-300 animate-pulse" size={64} />
